    ...
    func test(id: String, indexPath: NSIndexPath)
    {
        Alamofire.request(.GET, hwkListUrl.stringByReplacingOccurrencesOfString("||COURSEID||", withString: id)).responseString
        {
            ...
            case .Success:
                ...
                {
                    print(bbsResponseNSStr.substringWithRange(result.range))
                }
            case .Failure: break
            }
        }
                                        ...
                                        case .Success:
                                            ...
                                            dispatch_group_async(group, backgroundQueue, {
                                                ...
                                                for result in results
                                                {
                                                    ...
                                                    if eee.count > 5 {
                                                        let fff = eee[2].stringByReplacingOccurrencesOfString("course_", withString: "").stringByReplacingOccurrencesOfString("</a", withString: "").stringByReplacingOccurrencesOfString("&nbsp;", withString: "").componentsSeparatedByString("&id=")
                                                        if fff.count > 2 {
                                                            let bbs = BBS()
                                                            bbs.id = fff[1]
                                                            bbs.title = fff[2].componentsSeparatedByString(">")[1]
                                                            bbs.owner = eee[3].componentsSeparatedByString("</td")[0]
                                                            bbs.date = eee[4].componentsSeparatedByString("</td")[0]
                                                            bbs.isNew = eee[5].componentsSeparatedByString("</td")[0] == "未读"
                                                            bbs.course_id = id
                                                            try! realm.write
                                                                {
                                            ...
                                            dispatch_group_async(group, backgroundQueue, {
                                                ...
                                                for result in results
                                                {
                                                    let eee = hwkResponseNSStr.substringWithRange(result.range).stringByReplacingOccurrencesOfString("<fontcolor=red>", withString: "").stringByReplacingOccurrencesOfString("</font>", withString: "").stringByReplacingOccurrencesOfString("&nbsp;", withString: "").componentsSeparatedByString("</td><tdwidth=\"10%\">")
                                                    if eee.count > 2 {
                                                        let fff = eee[2].componentsSeparatedByString("</td><tdwidth=\"15%\">")
                                                        let ggg = eee[0].stringByReplacingOccurrencesOfString("&course_", withString: "").stringByReplacingOccurrencesOfString("&rec_", withString: "").stringByReplacingOccurrencesOfString("&rec_", withString: "").componentsSeparatedByString("id=")
                                                        if fff.count > 1 && ggg.count > 3 {
                                                            ...
                                                            if hhh.count > 1 {
                                                                ...
                                                                hwk.id = ggg[1]
                                                                hwk.rec_id = hhh[0]
                                                                hwk.title = hhh[1].stringByReplacingOccurrencesOfString("</a>", withString: "")
                                                                hwk.startDate = eee[1]
                                                                hwk.deadLine = fff[0]
...
//        let size = title.sizeThatFits(CGSizeMake(title.frame.width, CGFloat(MAXFLOAT)))
//        title.frame = CGRectMake(title.frame.x, title.frame.y, size.width, size.height)
//        self.frame.height = title.frame.height + owner.frame.height + 25
        if row.value!.isNew {
